...
Reviewer Receipt 还必须包含 `review_context_id`、Reviewer角色/模型身份、输入快照与证据包摘要、只读权限证明、开始结束时间以及实际读取的原始文件/diff范围。作者会话的总结只能作为待核线索，不能成为Reviewer唯一输入。Reviewer若参与了待审代码修改，该轮不再满足独立性，必须换新上下文重新审查。
