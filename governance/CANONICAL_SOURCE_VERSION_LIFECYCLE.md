...
Capability 的公开契约和规则只在 `capabilities/` 维护。DeepSeek Harness、Codex 或其他环境需要的配置、提示入口或生成副本只能是派生投影，并且必须记录：

- Canonical Source 路径、版本和内容摘要；
- 目标 Harness 与适配器版本；
- 生成或同步方式；
- 兼容矩阵状态及验证 Receipt；
- 漂移检查和删除/替换办法。

不得人工维护彼此独立的多份规则副本。投影与正本不一致时，投影失效并重新生成或重新适配，不能反向选择更顺眼的一份作为事实。
