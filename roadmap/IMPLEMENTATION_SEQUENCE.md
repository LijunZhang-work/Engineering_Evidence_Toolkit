...
等价入口是：用户在本机桌面控制台明确提交 `BUILD_MISSING + TOOLKIT_ONLY` 的目标工作集，
且生成的 `WorksetRequest` 通过 Schema、`intent_digest`、能力全集覆盖（纳入 + 排除）和 Runtime
仓外边界校验。此时授权只覆盖该工作集；不能把一次局部选择解释为全工具集建设确认。
...
1. 先读取当前 `WorksetRequest`；只在其选中能力闭包内寻找最早未通过的 Release Gate。没有具体工作集时，不得把宽泛的“持续推进”自动解释为建设全部能力。
2. 把缺口拆成可独立验证的小任务。
...
`B0–B12` 是依赖地图，不是一次运行必须从头扫到尾的全局队列。先按用户目标选最小工作集，
再在该闭包内按下面的依赖顺序推进；不属于本次工作集的阶段保持排除状态：
